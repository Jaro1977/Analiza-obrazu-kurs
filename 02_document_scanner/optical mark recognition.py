...
def prepare_image(image):
    ...
    thresh = cv2.threshold(src =edges, thresh = 50, maxval = 255, type = cv2.THRESH_BINARY | cv2.THRESH.OTSU)[1]
    return thresh
...
#klucz odpowiedzi
ANSWER_KEY = {0:0, 1:3, 2:0, 3:2, 4:1, 5:3, 6:4, 7:1, 8:3, 9:0}
#wczytanie obrazu
image = cv2.imread(args['image'])
#przygotowanie obrazu
thresh = prepare_image(image)

#wyszukanie wszystich krawędzi
...
for cnt in cnts:
    ...
    if w>50 and h >50 and aspect_ratio > 0.9 and aspect_ratio < 1.1:
        question_contour.append(cnt)

print(f'[INFO] Liczba wykrytych odpowiedzi: {len(question_contour)}')
...
correct = 0
question_contour = imutils.contours.sort_contours(question_contour, method = 'top-to-bottom')[0]

for question, idx in enumerate(range(0,len(question_contour), 5)):
    fields = question_contour[idx:idx+5]
    fields = imutils.contours.sort_contours(fields, method = 'left-to-right')
    marked = None
    ...
    for cnt_idx, cnt in enumerate(fields):
        ...
        cv2.drawContour(image = mask, contours = [cnt], contourIdx = -1, color = 255, thickness = -1)
...
score = (correct/10)
color = (50,168,82 if score > 0.6 else (77,7,219))
...
cv2.imwrite(r'checked.png', checked)
